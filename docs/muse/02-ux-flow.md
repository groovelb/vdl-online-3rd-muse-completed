...
- **분량**: 328줄(권장 250). 가독성 규칙(표 분할, 비고 목록, 구분선)으로 늘었다. 5절을 `appendix-screen-component-map.md`로 옮기면 줄일 수 있다.
...
- 프로젝트의 모드 값은 `concept`과 `system` 둘뿐이다. 코드 직행 모드는 폐기됐고 `system`이 그 산출물을 흡수했다.
- 분석 결과의 토큰마다 `decisionRationale`이 붙는다. 출처 레퍼런스, 고른 이유, 적용한 노트, 탈락 후보가 그 안에 들어간다.
- 토큰 편집이 건드리는 키는 `isEnabled`와 `emphasis` 둘이다. 레이아웃 토큰만 `columns`, `gap`, `px`, `maxWidth` 같은 치수 키를 더 갖는다.
