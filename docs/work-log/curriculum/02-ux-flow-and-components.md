...
AppShell
├─ Header (업로드 버튼)
├─ TagGroupFilter (3단 계층)
└─ InfiniteMasonry
    └─ ImageCard[] (dominantColors 포함)
```
...
```
SplitScreen 60:40
├─ 좌 60%: CategoryTab + TokenListItem[] (편집)
└─ 우 40%: 실시간 프리뷰
```
...
- [ ] ArchivePage 템플릿 (업로드+필터+무한그리드)
- [ ] ProjectDetailPage 템플릿 (SplitScreen 60:40)
- [ ] ProjectListPage 템플릿 (MoodboardCard grid)
- [ ] SettingsPage 템플릿 (4 섹션)
...
이 Stage의 출처:
- **002 일부** (컴포넌트 역할 7그룹) → §③ 컴포넌트 리스트
- **005** (Phase 1-5 일괄 구현) → Step 1-5
- **006** (ArchivePage, ProjectDetailPage 조립) → Step 8
- **007** (ProjectListPage, SettingsPage 추가) → Step 8
- **010 일부** (MoodboardCard 썸네일 매핑) → **Step 7로 당김**
- **021 일부** (ImageCard dominantColors, 필터 3단 계층) → **Step 6, Step 4로 당김**

### 실제 진행 이력 부록

> 원본 프로젝트에서 ImageCard의 `dominantColors` 오버레이는 **021**(디자인 QA 직전)에 추가됐다. 그 전까지는 ImageCard가 색 정보를 표시하지 않아서 Archive에서 "색으로 레퍼런스를 찾기 어렵다"는 피드백이 누적됐다. 필터의 3단 계층 구조(SuperSection/SubRow)도 같은 시점에 잡혔다.
>
> MoodboardCard의 썸네일 매핑 문제는 **010**에서 버그로 발견됐다. 당시 "MoodboardCard에 src prop을 추가할까" 논의가 있었지만, 다른 소비자 영향을 고려해 **호출측 어댑터(ProjectListPage)로 해결**했다.
>
