...
### Step 6. GNB 레이아웃 — MuseNav + UserMenu + AppShellLayout

**구조 원칙** (CRITICAL): GNB(헤더)는 **라우트 레이아웃 수준에서 딱 한 번**. 페이지 템플릿 안에 `<AppShell>` 을 넣지 않는다.

`src/components/layout/AppShell.jsx` — starter-kit 기본 컴포넌트로 이미 존재.  **내부에 GNB 를 내장**하고 `logo` / `headerPersistent` prop 을 받아 헤더를 구성한다. 우리는 이 AppShell 을 그대로 활용하고 MUSE 전용 콘텐츠(로고+nav, 유저 메뉴)를 prop 으로 주입한다.

`src/pages/MuseNav.jsx` — AppShell 의 `logo` slot 에 들어갈 좌측 영역:
- 로고(`MUSE`) + nav links (`/archive`, `/projects`, `/settings`)
- `react-router-dom` 의 `NavLink` 로 isActive 스타일

`src/pages/UserMenu.jsx` — AppShell 의 `headerPersistent` slot 에 들어갈 우측 영역:
- Avatar 클릭 시 이메일/로그아웃 dropdown
- Stage 5 에서 Auth 붙이기 전까진 `useAuth()` 가 null 이라 렌더 안 됨 (정상)

`src/pages/AppShellLayout.jsx` — **신규**. 라우트 공통 레이아웃 엘리먼트:
...
Stage 4 에서 `App.jsx` 의 라우트를 이 레이아웃으로 중첩한다:
```jsx
<Routes>
...
**규칙 (반드시 지킬 것)**:
- **페이지 템플릿에는 `<AppShell>` 금지**. 템플릿은 `<PageContainer>` 이하의 본문만 반환.
- GNB 는 `AppShellLayout` 에서만 렌더. 페이지 전환 시 헤더가 언마운트/리마운트 되지 않아야 Drawer 상태·포커스가 유지된다.
- 로고·nav 정의를 각 페이지가 중복해서 갖지 않는다.
...
- [ ] MuseNav + UserMenu 컴포넌트 (mock, Stage 5 전까지 UserMenu 는 빈 렌더)
- [ ] AppShellLayout 컴포넌트 — `src/pages/AppShellLayout.jsx` (Stage 4 라우팅에서 사용)
- [ ] **페이지 템플릿에 `<AppShell>` 넣지 않기** — AppShell 은 라우트 레이아웃이지 페이지가 아니다
...
> 실제 MUSE 프로젝트에서 **디자인 QA 전수 점검은 거의 마지막에 나왔다**. 토큰 세팅 당시 `MuiPaper` elevation=0 설정을 빠뜨려서, 이후 컴포넌트·페이지를 다 만든 뒤 QA 단계에서 **하나씩 elevation=0을 추가하는 작업**이 발생했다. hover 위치 이동, floating label, GNB 레이아웃도 같은 이유로 뒤늦게 정비됐다.
>
