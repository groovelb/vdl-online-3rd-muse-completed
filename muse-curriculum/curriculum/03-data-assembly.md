...
> 실제 MUSE 프로젝트의 데이터 모델은 **세 번의 큰 변화**를 겪었다:
>
> 1. **1차**: `Reference.tags`가 `string[]` (flat). 27건을 여기에 맞춰 생성.
> 2. **2차**: 사용자 제공 프리셋 JSON을 통합하며 **flat → 중첩 + enum**으로 전환. `flattenTags()` 어댑터를 만들어 기존 flat 가정 코드를 호환. `keyVisual` 이미지 보드 레이어가 `visualDirection` Markdown+태그로 교체.
> 3. **3차**: `Reference.extracted` 필드 신설. T1이 per-image extraction까지 하도록 역할 확장.
>
