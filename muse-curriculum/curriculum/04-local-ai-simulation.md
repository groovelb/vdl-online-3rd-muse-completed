...
### 라우팅 원칙
- **stateless 템플릿** (`components/templates/*.jsx`) — Storybook에서 props만으로 렌더. **AppShell 래핑 금지**
- **컨테이너 `*Route.jsx`** (`pages/*Route.jsx`) — store 주입, 템플릿에 props 전달
- **공통 레이아웃 `AppShellLayout.jsx`** — `<AppShell logo={<MuseNav/>} headerPersistent={<UserMenu/>}><Outlet/></AppShell>`. 라우트에서 중첩으로 공통 주입 → 페이지 전환해도 헤더 언마운트 안 됨
- 5 경로: `/archive`, `/projects`, `/projects/new`, `/projects/:id`, `/settings` (+`/auth` 는 별도, AppShellLayout 바깥)
...
### Step 9. React Router — App.jsx + AppShellLayout

**구조**: `<AuthGuard>` → `<AppShellLayout>` → `<Outlet/>` 로 중첩. Protected 라우트 전부가 이 레이아웃을 공유해서 **페이지 전환 시에도 헤더가 언마운트되지 않는다**.

```jsx
// src/pages/AppShellLayout.jsx  (Stage 1 Step 6 에서 이미 작성)
...
import {
  ArchiveRoute, ProjectListRoute, ProjectCreateRoute,
  ProjectDetailRoute, SettingsRoute, AppShellLayout,
} from './pages';
...
**함정 주의**:
- `<AppShellLayout>` 이 바깥 레이아웃 route 의 `element` 에 들어가야 함. 각 자식 route 의 element 로 넣으면 페이지 전환 시마다 AppShell 이 리마운트됨 (잘못된 구조).
- `/auth` 는 AppShellLayout **바깥** — 로그인 화면에 GNB 를 보이지 않음.
...
> 실제 MUSE 프로젝트의 AI 파트는 **가장 크게 재설계된 영역**이다:
>
> - **초기**: T1만 먼저 연결. 프롬프트·tool schema는 인라인.
> - **중기**: 태그 구조 재설계로 T1 tool schema 전면 개편 (enum 강제). T2/T3 추가. T3는 **이미지 포함 2-tool** 이었다.
> - **후기**: T3 이미지를 1024→512px로 축소 (17% 절감). 아직 이미지 포함 상태.
> - **최종 재설계**: **T1에 extraction 역할 추가, T3를 text-only로 전환**. Haiku로 통일. 비용 2.5배 절감. 이 시점에서 기존 Reference 데이터를 truncate.
>
> 업로드 flow 안정화도 별도 이슈였다:
> - 초기 구현 시 Promise.all() 사용. 20건 동시 업로드 시 간헐 누락.
> - rate limit + race 버그 재현 후 `runWithConcurrency(3)` + `withRetry(3회)` + 수동 재시도 🔄 도입.
>
> **교훈**:
