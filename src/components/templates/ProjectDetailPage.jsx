...
export function ProjectDetailPage({
  ...
  const handleDownloadConceptPrompt = () => {
    if (!conceptPrompt) return;
    const slug = (project?.name || 'concept').toLowerCase().replace(/[^\w가-힣\s-]/g, '').replace(/\s+/g, '-');
    ...
    a.href = url;
    a.download = `muse-${slug}-concept-${date}.md`;
            ...
            <Box sx={ { display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', mb: 1 } }>
              ...
              <Box sx={ { display: 'flex', gap: 1 } }>
                ...
                <Button
                  ...
                  onClick={ handleDownloadConceptPrompt }
                  disabled={ !conceptPrompt }
                >
                  .md 다운로드
                </Button>
