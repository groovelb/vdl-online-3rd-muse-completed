...
**Scale**: ${Object.entries(tokens?.spacing || {}).map(([k, v]) => `\`${k}: ${typeof v === 'number' ? `${v}px` : v}\``).join(' · ') || '(없음)'}
...
**Scale**: ${Object.entries(tokens?.rounded || {}).map(([k, v]) => `\`${k}: ${typeof v === 'number' ? `${v}px` : v}\``).join(' · ') || '(없음)'}

---

## 8. Elevation

${ld.elevation || ((tokens?.elevation || []).length === 0 ? '_(elevation 토큰 없음 — 평면 디자인)_' : '(상세 없음)')}

${(tokens?.elevation || []).length > 0 ? (tokens.elevation.map((e) => `- **${e.label || e.id}** (level ${e.level ?? 0}): \`${e.shadow}\``).join('\n')) : ''}
