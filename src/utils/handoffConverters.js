...
export function buildCursorRules({ project, tokens, layerDetails, references }) {
...
## Reference Images (각 ref 의 차용 부분 명시)
${attachmentTable}
...
- \`tailwind.config.js\` / \`mui-theme.js\` / \`tokens.css\` — framework 변환
...
export function buildDesignSystemMd({ project, tokens, visualDirection, layerDetails, references }) {
...
> 외부 AI 도구에 본 문서를 paste 할 때 위 파일명을 그대로 첨부하세요. 본문에서 \`ref-XXX\` 언급은 위 매칭 표의 파일과 일치합니다.
...
### Tailwind
\`tailwind.config.js\` 를 프로젝트 루트에 복사. \`module.exports.theme.extend\` 가 토큰을 노출.

### MUI v7
\`mui-theme.js\` import 후 \`<ThemeProvider theme={theme}>\` 로 감싸기.

### Pure CSS
\`tokens.css\` 를 entry 에 import. \`var(--color-primary)\` 등으로 사용.
